...
function profilepro_profile_details( $field, $user_id ) {
		global $profilepro;
...
function profilepro_update_user_profile( $user_id, $form_data, $action='new_user' ){
...
function profilepro_auto_login( $username, $remember=true ){
	ob_start();
...
function profilepro_get_edit_user(){
	if (is_user_logged_in()) {
...
function profilepro_get_view_user(){
	global $current_user;
	$current_user=wp_get_current_user();
	$user_id = $current_user->ID;
	return $user_id;
