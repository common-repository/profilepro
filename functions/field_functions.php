...
if( !class_exists( 'PROFILEPROFieldFunctions' ) ){
	
	class PROFILEPROFieldFunctions{
		...
		function edit_fields( $key, $array, $form_type = null, $user_id = null  ){
			...
			if( $array['type']=='image_upload' ){
				$default_image = PROFILEPRO_URL."assets/images/profiledefault.png";
				$value = "<img src='".$default_image."' class='default'>";
			}
			if (isset($user_id)){
				$value = profilepro_profile_details( $key, $user_id );



				if( $array['type']=='image_upload' ){
					if( empty( $value ) ){
						$value = PROFILEPRO_URL."assets/images/profiledefault.png";
					}
			...
			$field = '';
			$field .= "<div class='profilepro-field' data-key=".$array['meta_key'].">";
			$field .= "<div class='profilepro-label'>"; 
			if(!empty($array['icon']))
			...
			if(isset ($array['is_required']) && $array['is_required']==1 ) 
				$field .="<span class='profilepro-required'>*</span>";
			
			$field .= "</div><div class='profilepro-element'>";	
			$readonly = '';
			...
			switch( $array['type'] ){
				...
				case 'text':
					...
					$field .= "<input type='text' name='".$array['meta_key']."' id='".$array['meta_key']."'  value='".$value."' $data $readonly />";
					break;
				...
				case 'password':
					...
					if( $array['add_confirm_password_field'] && is_user_logged_in()){
						$field .= "<div class='profilepro-clear'></div>";
						$field .= "</div>";
						$field .= "</div><div class='profilepro-clear'></div>";
						$field .= "<div class='profilepro-field'>";
						$field .= "<div class='profilepro-label'>"; 
						$field .= "<label for='confirm_pass'>".__( 'Confirm Password', PROFILEPRO_PLUGIN_SLUG )."</label>";
						$field .= "</div><div class='profilepro-element'>";	
				...
				case 'image_upload':
						...
						$field .= "<div class='profilepro-image' data-remove_text='".__('Remove','profilepro')."'>".$value."</div>";
						$field .= "<div class='profilepro-image-upload' data-filetype='picture' data-allowed_extensions=".$allowed_types.">".$array['upload_button_text']."</div>";
						$field .= "<input data-required='".$array['is_required']."' type='hidden' name='".$array['meta_key']."' id='".$array['meta_key']."' value='".profilepro_profile_details( $key, $user_id )."' />";
					break;
				...
				case 'radio':
					...
					$field .= "<div class='profilepro-radio-container' data-required='".$array['is_required']."'>";
					foreach($array['edit_choices'] as $k=>$v) {
							
						$v = stripslashes($v);
						$field .= "<label class='profilepro-radio'><span";
				...
				case 'checkbox':
					if (isset($array['edit_choices'])){
					$field .= "<div class='profilepro-checkbox-container' data-required='".$array['is_required']."'>";
					foreach($array['edit_choices'] as $k=>$v) {
						$v = stripslashes($v);
						$field .= "<label class='profilepro-checkbox'><span";
		...
		function view_fields( $key, $array, $form_type = null, $user_id = null ){
			...
			if ($user_id){
				...
				if( !empty( $value ) ){
					...
					$field .="<span class='faicons' ></span>";
					$field .="<label for='$key'>".$array['label']."</label>";
					$field .= "</div><div class='profilepro-element'>$value";	
