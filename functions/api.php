...
class PROFILEPROApi{
		...
		function __construct(){
			add_action('wp_head',array($this,'profilepro_ajax_urls'));
			add_action('admin_head',array($this,'profilepro_ajax_urls'));
			add_action('init',  array($this, 'process_email_approve'), 9);
			$this->filter = 0;
		}
		
		function profilepro_ajax_urls(){
				?>
					<script type="text/javascript">
			
					var profilepro_ajax_url = '<?php echo admin_url('admin-ajax.php'); ?>';
		...
		function create_new_user( $user_login, $user_password, $user_email, $form_data ){
			...
			profilepro_update_user_profile( $user_id, $form_data, $action='new_user' );
			if( $user_id ){
			 return $user_id;
			}
		}
	...
	function permalink( $user_id=0, $request='profile', $option='profilepro_pages' ) {
		
		$pages = get_option( $option );
		if (isset($pages[$request]) && $this->page_exists($pages[$request]) ){
			$page_id = $pages[ $request ];
		} else {
			$default = get_option('profilepro_pages');
	...
	function profilepro_extract_profile_for_mail($user_id, $form) {
		$output = '';
		$customfieldarray = array();
		foreach($form as $k=>$v){
				$val = profilepro_profile_details($k, $user_id);
				if (is_array($val)){
					$val = implode(', ',$val);
				}
				$output .= $k . ': '. $val . "\r\n";
				$fieldarray['{PROFILEPRO_'.$k.'}'] = $val;
	...
	function email_approve($user_id, $user_pass, $form) {
		...
		update_user_meta($user_id, 'profilepro_account_verify', $new_account_salt);
		update_user_meta($user_id, 'profilepro_account_status', 'pending');
		update_user_meta($user_id, 'profilepro_pending_pass', $user_pass);
		update_user_meta($user_id, 'profilepro_pending_form', $form);
		profilepro_mail($user_id, 'verifyemail', null, $form );
	}
	
	function admin_approve($user_id, $user_pass, $form) {
		$new_account_salt = wp_generate_password( $length=20, $include_standard_special_chars=false );
		update_user_meta($user_id, 'profilepro_account_status', 'pending_admin');
		update_user_meta($user_id, 'profilepro_pending_pass', $user_pass);
		update_user_meta($user_id, 'profilepro_pending_form', $form);
		profilepro_mail($user_id, 'pendingadminapprove', null, $form );
	}
	...
	function activate($user_id){
		...
		profilepro_mail( $user_id, 'newaccount', null, $form );
	}	
