...
if( !class_exists( 'PROFILEPROAdminPostMetaBoxes' ) ){
	
	class PROFILEPROAdminPostMetaBoxes{
		...
		function add_post_metabox(){
			...
			if( $current_screen->id == 'profilepro_form'){
		...
		function add_metabox_form( $post_type, $post ){
			...
			//add_meta_box('profilepro-admin-form-type', __('Select Form Type'), array(&$this, 'prepare_metaboxes'), 'profilepro_form', 'normal', 'default');
			add_meta_box('profilepro-admin-form-builder', __('Form Builder'), array(&$this, 'prepare_metaboxes'), 'profilepro_form', 'normal', 'default');
			add_meta_box('profilepro-admin-form-shortcode', __('Shortcode'), array(&$this, 'prepare_metaboxes'), 'profilepro_form', 'side', 'default');
			$form_type = get_post_meta($post->ID,'profilepro_form_type',true);
			if( empty( $form_type ) || ( $form_type == 'register' ) ){
				add_meta_box('profilepro-admin-form-role', __('Form role'), array(&$this, 'prepare_metaboxes'), 'profilepro_form', 'side', 'default');
				add_meta_box('profilepro-admin-form-autologin', __('Autologin'), array(&$this, 'prepare_metaboxes'), 'profilepro_form', 'side', 'default');
		
			}
		}
		
		function prepare_metaboxes( $object, $box ){
			include_once PROFILEPRO_PATH . 'admin/templates/form/'. $box['id'] . '.php';
			wp_nonce_field( basename( __FILE__ ), 'profilepro_save_metabox_form_nonce' );
		}
		
		function save_metabox_form( $post_id, $post, $update ){
			if ( !isset( $_POST['profilepro_save_metabox_form_nonce'] ) || !wp_verify_nonce( $_POST['profilepro_save_metabox_form_nonce'], basename( __FILE__ ) ) ) 				return $post_id;
			
			if ( $post->post_type != 'profilepro_form' ) return $post_id;
			
			$post_type = get_post_type_object( $post->post_type );
			if ( !current_user_can( $post_type->cap->edit_post, $post_id ) ) return $post_id;
			
			if( isset( $_POST['profilepro_form_type'] ) ){
				update_post_meta( $post_id, 'profilepro_form_type', $_POST['profilepro_form_type'] );
			}
			
			if( isset( $_POST['profilepro_field_order'] ) ){
				update_post_meta( $post_id, 'profilepro_field_order', $_POST['profilepro_field_order'] );
			}
			
			if( isset( $_POST['profilepro_form_role'] ) ){
				update_post_meta( $post_id, 'profilepro_form_role', $_POST['profilepro_form_role'] );
			}
			
		if( isset( $_POST['profilepro_form_autologin'] ) ){
				update_post_meta( $post_id, 'profilepro_form_autologin', $_POST['profilepro_form_autologin'] );
			}
		}
	...
	new PROFILEPROAdminPostMetaBoxes();
}
